 ...
 * @return string Полный URL: get_media?key=...&sig=...&exp=...
 */
function sign_media_url(string $s3Key, int $ttl = 3600): string {
    ...
    return 'get_media?key=' . rawurlencode($s3Key) . '&sig=' . $sig . '&exp=' . $expires;
}
...
function decrypt_aes256gcm(string $cipherText, string $keyHex, string $ivHex): string|false
{
    ...
    @file_put_contents($logFile, $ts . "START cipherLen=" . strlen($cipherText)
        . " keyHexLen=" . strlen($keyHex) . " ivHexLen=" . strlen($ivHex) . "\n", FILE_APPEND);

    if ($keyBin === false || strlen($keyBin) !== 32) {
        @file_put_contents($logFile, $ts . "FAIL bad key len=" . strlen($keyBin ?? '') . "\n", FILE_APPEND);
        return false;
    }
    if ($ivBin === false || strlen($ivBin) !== 12) {
        @file_put_contents($logFile, $ts . "FAIL bad IV len=" . strlen($ivBin ?? '') . "\n", FILE_APPEND);
        return false;
    }

    // GCM tag (16 bytes) is appended at the end by Web Crypto API
    $tagLen = 16;
    $ctLen  = strlen($cipherText);

    if ($ctLen <= $tagLen) {
        @file_put_contents($logFile, $ts . "FAIL ciphertext too short ($ctLen bytes)\n", FILE_APPEND);
        return false;
    }

    $tag = substr($cipherText, $ctLen - $tagLen, $tagLen);
    $ct  = substr($cipherText, 0, $ctLen - $tagLen);

    @file_put_contents($logFile, $ts . "split ctLen=" . strlen($ct) . " tagLen=" . strlen($tag)
        . " ctFirst16=" . bin2hex(substr($ct, 0, 16)) . "\n", FILE_APPEND);

    $result = @openssl_decrypt($ct, 'aes-256-gcm', $keyBin, OPENSSL_RAW_DATA, $ivBin, $tag);

    if ($result === false) {
        $err = openssl_error_string() ?: 'unknown';
        @file_put_contents($logFile, $ts . "FAIL openssl: " . $err . "\n", FILE_APPEND);
        return false;
    }

    @file_put_contents($logFile, $ts . "OK resultLen=" . strlen($result)
        . " firstBytes=" . bin2hex(substr($result, 0, 16)) . "\n", FILE_APPEND);
