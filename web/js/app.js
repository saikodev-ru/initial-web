...
async function _ssePoll(chatId) {
  ...
  if (_sseCursor <= 0) {
    try {
      var init = await fetch(API + '/poll_updates?cursor=0&chat_id=' + chatId + '&last_call_id=' + _sseCallLastId, {
  ...
  while (_sseActive && S.token) {
    try {
      ...
      var res = await fetch(
        API + '/poll_updates?cursor=' + _sseCursor + '&chat_id=' + chatId + '&last_call_id=' + _sseCallLastId,
        {
          headers: S.token ? { 'Authorization': 'Bearer ' + S.token } : {},
